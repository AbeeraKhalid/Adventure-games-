...
do {
    ...
    if (Opposer.Select == "SKELETON_GAME") {
        console.log((`${chalk.bold.blueBright(P1.name)} VS ${chalk.bold.green(O1.name)}`));
        ...
        if (tell.options == "ATTACK") {
            ...
            if (Num > 0) {
                ...
                if (P1.fuel <= 0) {
                    console.log(chalk.redBright.bold.italic("You loose this game,BestOfLuckForTheNextTime!!!"));
                    process.exit();
                }
            }
            if (Num <= 0) {
                ...
                if (O1.fuel <= 0) {
                    console.log(chalk.green.bold.italic("Congratulation!You are a Winner***"));
                    process.exit();
                }
            }
        }
        if (tell.options == "DRINK_PORTION") {
            P1.fuelIncrease();
            console.log(chalk.italic.green(`your Drink_HealthPortion & Your Fuel is${P1.fuel}`));
        }
    ...
    if (Opposer.Select == "HUNTER_ASSASIN_GAME") {
        console.log((`${chalk.bold.magentaBright(P1.name)} VS ${chalk.bold.green(O1.name)}`));
        let tell = await inquirer.prompt({
            name: "options",
            type: "list",
            message: "Select your opponent:",
